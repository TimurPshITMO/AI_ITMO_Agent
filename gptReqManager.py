...
def gptReq(sdk, instructions, prompt, log=lambda x:print(x)):
    ...
    """
    функция для создания запроса к ЯндексГПТ
    """
...
def checkValid(sdk, prompt, log):
    ...
    Осуществляет проверку корректности запроса пользователя (относится ли к ИТМО, нет ли попытки обмана и тд)
    """
...
def firstTry(sdk, prompt, log):
    ...
    Определяет, возможно ли дать хороший ответ на вопрос "сходу", без дополнительного поиска
    """
...
def outSearch(sdk, prompt, log, prevlink=0, counter=0):
    ...
    Последовательный анализ наиболее "подходящих" ссылок (в том числе внутренних) и их замешивание в контекст
    """
...
def classify(sdk, userPrompt, answer, log):
    ...
    """
    Четвертый агент (завершающий):
    
    Помогает выделить нужные для json поля, а также "резюмирует" мотивацию (reasoning)
    """
    ...
    result = {'answer' : correct_id, 'reasoning' : ans[ans.find('\n'):] if ans.find('\n') else ans, 'sources': links}
